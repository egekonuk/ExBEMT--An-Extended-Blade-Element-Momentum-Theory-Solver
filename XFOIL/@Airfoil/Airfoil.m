classdef Airfoil < handle
    ...
    methods
        ...
        function plot(this,ViewPoints)
            global x_blade iy beta c R
%             plot(this.UpperX,this.UpperY,'-r');
%             hold on
%             plot(this.LowerX,this.LowerY,'-r');
%             hold on
             X = [this.UpperX ; flipud(this.LowerX)].*c(iy);
             Y = [this.UpperY ; flipud(this.LowerY)].*c(iy);
             Z = ones(length(X),1)*x_blade(iy)*R;
            
            h = plot3(X,Z,Y,'-r',LineWidth=1.5);
            direction = [0 1 0];
            rotate(h,direction,beta(iy))
